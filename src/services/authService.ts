...
export const logout = async (): Promise<void> => {
    ...
    if (refreshToken) {
        try {
            await apiClient.post(`${API_URL}/logout`, { refreshToken });
        } catch (error) {
...
export const verifyForgotPasswordOTP = async (email: string, otp: string) => {
    const response = await apiClient.post(`${API_URL}/verify-reset-otp`, { email, otp });
    return response.data;
};

export const resetPassword = async (data: any) => {
    const response = await apiClient.post(`${API_URL}/reset-password`, data);
    return response.data;
};
